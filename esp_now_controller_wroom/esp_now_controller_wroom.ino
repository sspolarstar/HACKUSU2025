...
const JoystickCalibration X1_CAL = {0, 4096, 82};    // X1 has 0-3200 range
const JoystickCalibration Y1_CAL = {0, 4096, 84};    // Y1 has 100-4100 range
const JoystickCalibration X2_CAL = {0, 4096, 90};    // Y2 has 0-3740 range
const JoystickCalibration Y2_CAL = {0, 4096, 70};    // Y2 has 0-3740 range
...
void loop() {
  ...
  data.x2 = readJoystick(PIN_ANALOG_X2, X2_CAL);
  data.y2 = readJoystick(PIN_ANALOG_Y2, Y2_CAL);

  // Serial.printf("x1: %4d|\t y1: %4d|\tx2: %4d|\t y2: %4d|\t\n", data.x1, data.y1, data.x2, data.y2);
  // Serial.printf("StickR: %d | StickL: %d | BumpL: %d, Bumpr: %d\n\n\n", data.stickR, data.stickL, data.bumpR, data.bumpL);
